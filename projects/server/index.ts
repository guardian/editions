...
import { ItemResponseCodec } from "typescript-capi-thrift-experiment/codegen";
const key = process.env.capikey
const app = express()
const port = 3131
const url =( path:string) => `https://content.guardianapis.com/${path}?format=thrift&api-key=${key}&show-elements=all&show-atoms=all&show-rights=all&show-fields=all&show-tags=all&show-blocks=all&show-references=all`
