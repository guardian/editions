...
import { BufferedTransport, CompactProtocol } from "@creditkarma/thrift-server-core";
   import {ItemResponseCodec} from "capi-ts" 
const app = express()
...
const port = 3131
const url = (path: string) => `https://content.guardianapis.com/${path}?format=thrift&api-key=${process.env.capikey}&show-elements=all&show-atoms=all&show-rights=all&show-fields=all&show-tags=all&show-blocks=all&show-references=all`

 const getArticle = async (path: string) => {
    const resp = await fetch(url(path))
    const buffer = await resp.arrayBuffer()

     const receiver: BufferedTransport = BufferedTransport.receiver(new Buffer(buffer))
    ...
    const body = data&&data.content&&data.content.blocks&&data.content.blocks.body&&data.content.blocks.body.map(_ => _.elements)
    return [title, body]
