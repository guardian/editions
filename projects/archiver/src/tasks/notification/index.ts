...
export const handler: Handler<NotificationTaskInput, NotificationTaskOutput> =
    handleAndNotify(
        'notified',
        async ({ issuePublication, issue }) => {
            console.log(
                `Scheduling notification for ${issue.name}, ${issue.date}`,
            )
            await sleep(1000)

            const stage: string = process.env.stage || 'code'

            const { issueDate, edition, notificationUTCOffset, topic } =
                issuePublication
